...
class TestELP(TestCase):
    def test_dimension3(self):
        ...
        np.testing.assert_array_almost_equal(
            calc_longitude, _comp_longitude, decimal=2)
        np.testing.assert_array_almost_equal(
            calc_longitude - _comp_longitude, -4.05 / 1000, decimal=0)
        np.testing.assert_array_almost_equal(
            calc_latitude, _comp_latitude, decimal=5)
        np.testing.assert_array_almost_equal(
            calc_radius, _comp_radius, decimal=1)
    ...
    def test_dimension3_01(self):
        ...
        np.testing.assert_array_almost_equal(
            calc_longitude, 264.8092609, decimal=5)
        np.testing.assert_array_almost_equal(
            calc_latitude, 3.2272014, decimal=5)
    ...
    def test_arrays(self):
        ...
        np.testing.assert_array_almost_equal(
            calc_longitude - _comp_longitude, [4.61 / 1000, 131600. / 1000], decimal=0)
        np.testing.assert_array_almost_equal(
    ...
    def test_compare_to_schureman(self):
        ...
        for i in range(len(Nv)):
            np.testing.assert_array_almost_equal(
                Nv[i], Nv_schureman[i], decimal=1)
            np.testing.assert_array_almost_equal(
                p[i], p_schureman[i], decimal=1)
            np.testing.assert_array_almost_equal(
                s[i], s_schureman[i], decimal=1)
