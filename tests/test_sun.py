...
class TestSun(TestCase):
    ...
    def test_dimension3(self):
        ...
        np.testing.assert_array_almost_equal(r_to_d(L) * 3600, 199.907372 * 3600, decimal=0)
        np.testing.assert_array_almost_equal(r_to_d(B) * 3600, 0.644, decimal=1)
        np.testing.assert_array_almost_equal(R * km_per_au, 0.99760775 * km_per_au, decimal=-3)
        L, B = vsop_to_fk5(2448908.5, L, B)
        np.testing.assert_array_almost_equal(r_to_d(L) * 3600, 199.907347 * 3600, decimal=0)
        np.testing.assert_array_almost_equal(r_to_d(B) * 3600, 0.62, decimal=0)
        aberration = aberration_low(R)
        np.testing.assert_array_almost_equal(r_to_d(aberration) * 3600, -20.539, decimal=3)

        np.testing.assert_array_almost_equal(r_to_d(L) * 3600 * 100, dms_to_d(199, 54, 26.18) * 3600 * 100, decimal=1)
        np.testing.assert_array_almost_equal(r_to_d(B) * 3600 * 100, 0.72 * 100, decimal=1)
