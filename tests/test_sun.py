...
class TestSun(TestCase):
    ...
    def test_compare_to_schureman(self):
        ...
        for i in range(len(h)):
            np.testing.assert_array_almost_equal(
                h[i], h_schureman[i], decimal=1)
            np.testing.assert_array_almost_equal(
                p1[i], p1_schureman[i], decimal=1)
